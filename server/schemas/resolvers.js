...
const resolvers = {
  Query: {
    ...
    allSnippets: async (parent, { tags }) => {
      try {
        ...
        error //catches any errors that occur, log it to console, & throw it as a new error
      ) {
        console.error(error);
    ...
    userSnippets: async (parent, { username, tags }) => {
      try {
        ...
        const filter =
          tags && tags.length > 0
            ? { tags: { $all: tags }, username: username }
            : { username };
        ...
        console.error(error);
        throw new Error(
          "Failed to retrieve the user's snippets; " + error.message
        );
      }
    },
    ...
    userSavedSnippets: async (parent, { username }) => {
      try {
        ...
        console.error(error);
        throw new Error(
          "Failed to retrieve the user's saved snippets; " + error.message
        );
      }
    },
    ...
    userComments: async (parent, { username }) => {
      try {
        ...
        const comments = await Comment.find({ username }).populate(
          "parentSnippetId"
        );
        return comments;
      } catch (
        ...
        console.error(error);
        throw new Error(
          "Failed to retrieve the user's comments; " + error.message
        );
      }
    },
    ...
    oneSnippet: async (parent, { snippetId }) => {
      try {
        ...
        const snippet = await Snippet.findOne({ _id: snippetId }).populate(
          "comments"
        );
        return snippet;
      } catch (
  ...
  Mutation: {
    ...
    editUser: async (
      parent,
      { currentUser, username, password, image, currentPassword }
    ) => {
      try {
        //about to find user
        const userToUpdate = await User.findOne({ username: currentUser });

        //about to compare passwords
        const passwordComparisonResult = await userToUpdate.comparePassword(
          currentPassword
        );
    ...
    createSnippet: async (
      parent,
      { username, snippetTitle, snippetText, snippetCode, resources, tags }
    ) => {
      try {
    ...
    editSnippet: async (
      parent,
      { snippetId, snippetTitle, snippetText, snippetCode, resources, tags }
    ) => {
      try {
    ...
    deleteSnippet: async (parent, { snippetId }) => {
      try {
        ...
        await User.findOneAndUpdate(
          { username: deletedSnippet.username },
          {
    ...
    createComment: async (
      parent,
      { parentSnippetId, username, commentText }
    ) => {
      try {
    ...
    deleteComment: async (parent, { commentId }) => {
      try {
        ...
        await Snippet.findOneAndUpdate(
          { _id: deletedComment.parentSnippetId },
          {
            $pull: { comments: commentId },
          }
        );
